...
function nextLoad(n) {
    ...
    oImg.onload = function () {
        ...
        } else if (oImg.width - oImg.height > 0) {
...
function tabImg() {
    ...
    //算可移动范围：
    p.moveMax = p.sm.ul.offsetWidth - p.sm.div.offsetWidth;
    //处理下面当前项 跟着动
    //    console.log(p.sm.ul.offsetLeft,-p.moveMax + 118);
    
    var cv = Math.round(p.sm.div.offsetWidth / (p.sm.li[0].offsetWidth + 18));
    var three = cv - 3;
    ...
    if(p.now < p.sm.li.length - three && p.now>=2){
        p.sm.ul.style.left = -118 * (p.now - 2) + 'px';
    }
